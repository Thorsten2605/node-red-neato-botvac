...
/**
 * A clearCurrentCallback callback
 * @callback clearCurrentCallback
 * @param {string} [error] - An error message if an error occur
 * @param {string} result - "ok" on success or the request result if an error occur
 */

/**
 * Clear the current robot alert
 *
 * @param {clearCurrentCallback} callback - a callback called when executing the action or an error
 */
Robot.prototype.dismissCurrentAlert = function dismissCurrentAlert(callback) {
    var params = {};
    doAction(this, 'dismissCurrentAlert', null, function (error, result) {
        if (typeof callback === 'function') {
            if (error) {
                callback(error, result);
            } else if (result && 'result' in result && result.result === 'ok') {
